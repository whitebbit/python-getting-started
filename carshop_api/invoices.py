...
def get_monobank_public_key():
    r = requests.get("https://api.monobank.ua/api/merchant/pubkey", headers={"X-Token": settings.MONOBANK_TOKEN})
    r.raise_for_status()
    return r.json()["key"]
...
def verify_signature(request):
    ok = _verify_signature(
        request.headers["X-Sign"], request.body, MonoSettings.get_latest_or_add(get_monobank_public_key).public_key
    )
...
def create_invoice(order: Order, webhook_url):
    ...
    for order_quantity in order.car_types.all():
        ...
        amount += sum_
        basket_order.append(
            {"name": order_quantity.car_type.name, "qty": order_quantity.quantity, "sum": sum_, "unit": "шт."}
        )
    merchants_info = {"reference": str(order.id), "destination": "Покупка машин", "basketOrder": basket_order}
